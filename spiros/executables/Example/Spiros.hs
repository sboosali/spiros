...
data Options = Options
  ...
  { doPrintVersion         :: Bool
  , doPrintLicense         :: Bool
  , doPrintInformation     :: Bool
  , doResolveConfiguration :: Bool
...
printVersion options@Options{..} = do
  ...
  when (verbosity >= Verbose) $ do
    ...
    let versionLine = mconcat [ (application & name), ", version ", versionString ]
...
printLicense options@Options{..} = do
...
printInformation options@Options{..} = do
  ...
  when (verbosity == Concise) $ do
    ...
    putStr "Application Information = "
    print application

    putStr "Config Directory        = "
    printConfigurationDirectory

    putStr "Data Directory          = "

    printDataDirectory
    putStr "Cache Directory         = "
...
printConfigurationDirectory :: IO ()
printConfigurationDirectory = do
...
printDataDirectory :: IO ()
printDataDirectory = do
...
printCacheDirectory :: IO ()
printCacheDirectory = do
...
resolveConfig = do
  ...
  fpConfig <- getConfigFile "Config.hs"

  sConfig <- (Prelude.readFile fpConfig) `E.catch` (\(_e :: E.IOException) -> return "")

  let config = parseConfig sConfig
...
getVersionString = do
  ...
  evLongVersion <- IO.lookupEnv "LongVersion"

  --TODO...
  -- currentTimestamp <- _ -- shell "date +%Y%m%d%H%M"
  -- currentGitCommit <- _ -- shell "git rev-parse --verify HEAD"

  let versionStringTags = evLongVersion & maybe "" parseVersionTags
  ...
  parseVersionTags s0 = s2
    where

    s1 = s0 & filter (not . Char.isSpace)
    s2 = if List.null s1 then "" else ("-" ++ s1)
...
options = do
  ...
  doPrintVersion <- P.switch (mconcat
  ...
  doPrintLicense <- P.switch (mconcat
  ...
  doPrintInformation <- P.switch (mconcat
  ...
  doResolveConfiguration <- P.switch (mconcat

        [ P.long  "config"
        , P.short 'c'
        , P.help  "Print the “resolved” configuration."
        , P.style P.bold
        ])
...
getConfigFile :: FilePath -> IO FilePath
getConfigFile = getApplicationSpecificConfigFile application

--------------------------------------------------

getDataFile :: FilePath -> IO FilePath
getDataFile = getApplicationSpecificDataFile application

--------------------------------------------------

getCacheFile :: FilePath -> IO FilePath
getCacheFile = getApplicationSpecificCacheFile application
